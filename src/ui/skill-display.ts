/**
 * Skill Display MCP App - Vanilla JS implementation
 * With Tabs (My Skills, Team, Community) and Cluster grouping
 */
import {
  App,
  applyDocumentTheme,
  applyHostStyleVariables,
  applyHostFonts,
} from "@modelcontextprotocol/ext-apps";

// Types
interface SkillDisplayInfo {
  name: string;
  description: string;
  path: string;
  assistantInvocable: boolean;
  userInvocable: boolean;
  isAssistantOverridden: boolean;
  isUserOverridden: boolean;
  // Source information
  sourceType: "local" | "github" | "bundled" | "well-known";
  sourceDisplayName: string;
  sourceOwner?: string;
  sourceRepo?: string;
}

interface SkillDisplayState {
  skills: SkillDisplayInfo[];
  totalCount: number;
  success?: boolean;
  error?: string;
}

type ActiveTab = "my-skills" | "team" | "community";

// State
let skills: SkillDisplayInfo[] = [];
let searchQuery = "";
let activeTab: ActiveTab = "my-skills";
let expandedClusters: Set<string> = new Set();
let hasAutoExpanded = false;
let app: App | null = null;

// DOM Elements
const skillClusters = document.getElementById("skill-clusters")!;
const stats = document.getElementById("stats")!;
const searchInput = document.getElementById("search-input") as HTMLInputElement;
const toast = document.getElementById("toast")!;
const tabsContainer = document.getElementById("tabs")!;
const tabCountMy = document.getElementById("tab-count-my")!;
const tabCountTeam = document.getElementById("tab-count-team")!;
const teamSkillList = document.getElementById("team-skill-list")!;

// HITL Modal Elements
const hitlModal = document.getElementById("hitl-modal")!;
const hitlMessage = document.getElementById("hitl-message")!;
const hitlApprove = document.getElementById("hitl-approve")!;
const hitlDeny = document.getElementById("hitl-deny")!;
let pendingHitlRequest: { skillName: string; enabled: boolean } | null = null;

// Mock team skills (to be replaced with API calls to Sanfran.md platform)
const TEAM_SKILLS: SkillDisplayInfo[] = [
  {
    name: "nda-summary",
    description: "Gera resumos estruturados de acordos de confidencialidade (NDAs) com extração de cláusulas-chave, prazos e obrigações.",
    path: "team://sanfran.md/nda-summary",
    assistantInvocable: true,
    userInvocable: true,
    isAssistantOverridden: false,
    isUserOverridden: false,
    sourceType: "bundled",
    sourceDisplayName: "Time",
  },
  {
    name: "contract-analysis",
    description: "Analisa contratos identificando riscos, cláusulas críticas e termos ausentes. Essencial para due diligence e revisão contratual.",
    path: "team://sanfran.md/contract-analysis",
    assistantInvocable: true,
    userInvocable: true,
    isAssistantOverridden: false,
    isUserOverridden: false,
    sourceType: "bundled",
    sourceDisplayName: "Time",
  },
  {
    name: "legal-research",
    description: "Pesquisa jurisprudência e doutrina por jurisdição e tema. Gera relatórios estruturados com citações verificadas.",
    path: "team://sanfran.md/legal-research",
    assistantInvocable: true,
    userInvocable: true,
    isAssistantOverridden: false,
    isUserOverridden: false,
    sourceType: "bundled",
    sourceDisplayName: "Time",
  },
];

// Handle host context changes
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function handleHostContextChanged(ctx: any) {
  if (ctx.theme) {
    applyDocumentTheme(ctx.theme);
  }
  if (ctx.styles?.variables) {
    applyHostStyleVariables(ctx.styles.variables);
  }
  if (ctx.styles?.css?.fonts) {
    applyHostFonts(ctx.styles.css.fonts);
  }
  // Handle safe area insets for mobile/notched devices
  if (ctx.safeAreaInsets) {
    const { top, right, bottom, left } = ctx.safeAreaInsets;
    document.body.style.paddingTop = `${top + 16}px`;
    document.body.style.paddingRight = `${right + 16}px`;
    document.body.style.paddingBottom = `${bottom + 16}px`;
    document.body.style.paddingLeft = `${left + 16}px`;
  }
}

// Update state from tool result
function updateState(data: SkillDisplayState) {
  if (data.skills) {
    skills = data.skills;
  }
  render();
}

// Extract category from skill path
// e.g. ".../legal-doc-library/Drafting/motion-to-dismiss/SKILL.md" → "Drafting"
// e.g. ".../skills/some-skill/SKILL.md" → "Sem Categoria"
function extractCategory(skillPath: string): string {
  const normalized = skillPath.replace(/\\/g, "/");
  const knownCategories = ["Research", "Analysis", "Drafting", "Citation", "Practice Management", "General"];

  for (const cat of knownCategories) {
    if (normalized.includes(`/${cat}/`)) {
      return cat;
    }
  }
  return "Outros";
}

// Get filtered skills based on search query
function getFilteredSkills(): SkillDisplayInfo[] {
  if (!searchQuery) {
    return skills;
  }
  const query = searchQuery.toLowerCase();
  return skills.filter(
    (skill) =>
      skill.name.toLowerCase().includes(query) ||
      skill.description.toLowerCase().includes(query)
  );
}

// Group skills by category
function groupByCategory(skillList: SkillDisplayInfo[]): Map<string, SkillDisplayInfo[]> {
  const groups = new Map<string, SkillDisplayInfo[]>();

  // Define display order
  const order = ["Drafting", "Analysis", "Practice Management", "Research", "Citation", "General", "Outros"];

  for (const skill of skillList) {
    const cat = extractCategory(skill.path);
    if (!groups.has(cat)) {
      groups.set(cat, []);
    }
    groups.get(cat)!.push(skill);
  }

  // Return in order
  const ordered = new Map<string, SkillDisplayInfo[]>();
  for (const cat of order) {
    if (groups.has(cat)) {
      ordered.set(cat, groups.get(cat)!);
    }
  }
  // Add any remaining
  for (const [cat, list] of groups) {
    if (!ordered.has(cat)) {
      ordered.set(cat, list);
    }
  }

  return ordered;
}

// Category display names (PT-BR)
const CATEGORY_LABELS: Record<string, string> = {
  "Drafting": "Elaboração de Documentos",
  "Analysis": "Análise & Revisão",
  "Practice Management": "Gestão de Casos",
  "Research": "Pesquisa Jurídica",
  "Citation": "Citação & Formatação",
  "General": "Geral",
  "Outros": "Outros",
};

// Category icons
const CATEGORY_ICONS: Record<string, string> = {
  "Drafting": "📝",
  "Analysis": "🔍",
  "Practice Management": "📋",
  "Research": "📚",
  "Citation": "📖",
  "General": "⚖️",
  "Outros": "📁",
};

// Render the UI
function render() {
  renderStats();
  if (activeTab === "my-skills") {
    renderMySkills();
  } else if (activeTab === "team") {
    renderTeamSkills();
  }
  updateTabCounts();
}

function updateTabCounts() {
  tabCountMy.textContent = String(skills.length);
  tabCountTeam.textContent = String(TEAM_SKILLS.length);
}

function renderStats() {
  const filtered = getFilteredSkills();
  if (searchQuery) {
    stats.textContent = `${filtered.length} de ${skills.length} skills`;
  } else {
    stats.textContent = `${skills.length} skill${skills.length !== 1 ? "s" : ""} disponíveis`;
  }

  const warningBanner = document.getElementById("skill-count-warning")!;
  if (skills.length >= 50) {
    warningBanner.style.display = "block";
  } else {
    warningBanner.style.display = "none";
  }
}

function renderMySkills() {
  const filtered = getFilteredSkills();

  if (skills.length === 0) {
    skillClusters.innerHTML = `
      <div class="empty-state">
        <p>Nenhuma skill disponível.</p>
        <p>Configure diretórios de skills usando a ferramenta skill-config.</p>
      </div>
    `;
    return;
  }

  if (filtered.length === 0) {
    skillClusters.innerHTML = `
      <div class="empty-state">
        <p>Nenhuma skill encontrada para sua busca.</p>
      </div>
    `;
    return;
  }

  const grouped = groupByCategory(filtered);

  // Auto-expand first cluster if none is expanded (only once)
  if (!hasAutoExpanded && grouped.size > 0) {
    expandedClusters.add(grouped.keys().next().value!);
    hasAutoExpanded = true;
  }

  let html = "";
  for (const [category, categorySkills] of grouped) {
    const isExpanded = expandedClusters.has(category);
    const label = CATEGORY_LABELS[category] || category;
    const icon = CATEGORY_ICONS[category] || "📁";

    html += `
      <div class="cluster ${isExpanded ? "expanded" : ""}" data-cluster="${escapeHtml(category)}">
        <div class="cluster-header" data-cluster="${escapeHtml(category)}">
          <div class="cluster-header-left">
            <span class="cluster-arrow">▶</span>
            <span>${icon}</span>
            <span class="cluster-name">${escapeHtml(label)}</span>
          </div>
          <span class="cluster-count">${categorySkills.length}</span>
        </div>
        <div class="cluster-body">
          <div class="skill-list">
            ${categorySkills.map((skill) => renderSkillCard(skill, false)).join("")}
          </div>
        </div>
      </div>
    `;
  }

  skillClusters.innerHTML = html;

  // Attach cluster toggle handlers
  skillClusters.querySelectorAll(".cluster-header").forEach((header) => {
    header.addEventListener("click", () => {
      const cat = (header as HTMLElement).dataset.cluster!;
      if (expandedClusters.has(cat)) {
        expandedClusters.delete(cat);
      } else {
        expandedClusters.add(cat);
      }
      renderMySkills();
    });
  });

  // Attach toggle and reset handlers
  attachSkillHandlers(skillClusters);
}

function renderTeamSkills() {
  if (TEAM_SKILLS.length === 0) {
    teamSkillList.innerHTML = `
      <div class="empty-state">
        <p>Nenhuma skill de time configurada.</p>
        <p>O administrador do seu time pode gerenciar skills obrigatórias pela plataforma Sanfran.md.</p>
      </div>
    `;
    return;
  }

  // Filter by search
  let filtered = TEAM_SKILLS;
  if (searchQuery) {
    const query = searchQuery.toLowerCase();
    filtered = TEAM_SKILLS.filter(
      (s) =>
        s.name.toLowerCase().includes(query) ||
        s.description.toLowerCase().includes(query)
    );
  }

  if (filtered.length === 0) {
    teamSkillList.innerHTML = `
      <div class="empty-state">
        <p>Nenhuma skill de time encontrada para sua busca.</p>
      </div>
    `;
    return;
  }

  teamSkillList.innerHTML = filtered
    .map((skill) => renderSkillCard(skill, true))
    .join("");
}

function renderSkillCard(skill: SkillDisplayInfo, isTeam: boolean): string {
  const isCustomized = skill.isAssistantOverridden || skill.isUserOverridden;
  // Build source badge based on type
  let sourceBadge: string;
  if (isTeam) {
    sourceBadge = `<span class="source-badge mandatory" title="Skill obrigatória do time">
         <svg class="source-icon" viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
           <path fill="currentColor" d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2zm3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/>
         </svg>
         Obrigatória
       </span>`;
  } else if (skill.sourceType === "github") {
    sourceBadge = `<span class="source-badge github" title="From GitHub: ${escapeHtml(skill.sourceDisplayName)}">
         <svg class="source-icon" viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
           <path fill="currentColor" d="M8 0C3.58 0 0 3.58 0 8c0 3.54 2.29 6.53 5.47 7.59.4.07.55-.17.55-.38 0-.19-.01-.82-.01-1.49-2.01.37-2.53-.49-2.69-.94-.09-.23-.48-.94-.82-1.13-.28-.15-.68-.52-.01-.53.63-.01 1.08.58 1.23.82.72 1.21 1.87.87 2.33.66.07-.52.28-.87.51-1.07-1.78-.2-3.64-.89-3.64-3.95 0-.87.31-1.59.82-2.15-.08-.2-.36-1.02.08-2.12 0 0 .67-.21 2.2.82.64-.18 1.32-.27 2-.27.68 0 1.36.09 2 .27 1.53-1.04 2.2-.82 2.2-.82.44 1.1.16 1.92.08 2.12.51.56.82 1.27.82 2.15 0 3.07-1.87 3.75-3.65 3.95.29.25.54.73.54 1.48 0 1.07-.01 1.93-.01 2.2 0 .21.15.46.55.38A8.013 8.013 0 0016 8c0-4.42-3.58-8-8-8z"/>
         </svg>
         ${escapeHtml(skill.sourceDisplayName)}
       </span>`;
  } else if (skill.sourceType === "bundled") {
    sourceBadge = `<span class="source-badge bundled" title="Bundled with server">
         <svg class="source-icon" viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
           <path fill="currentColor" d="M8.878.392a1.75 1.75 0 0 0-1.756 0l-5.25 3.045A1.75 1.75 0 0 0 1 4.951v6.098c0 .624.332 1.2.872 1.514l5.25 3.045a1.75 1.75 0 0 0 1.756 0l5.25-3.045c.54-.313.872-.89.872-1.514V4.951c0-.624-.332-1.2-.872-1.514L8.878.392ZM8 3.5a.75.75 0 0 1 .75.75v3.75a.75.75 0 0 1-1.5 0V4.25A.75.75 0 0 1 8 3.5Zm0 8a1 1 0 1 1 0-2 1 1 0 0 1 0 2Z"/>
         </svg>
         Bundled
       </span>`;
  } else if (skill.sourceType === "well-known") {
    sourceBadge = `<span class="source-badge well-known" title="From well-known publisher: ${escapeHtml(skill.sourceDisplayName)}">
         <svg class="source-icon" viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
           <path fill="currentColor" d="M8 0a8 8 0 100 16A8 8 0 008 0z"/>
         </svg>
         ${escapeHtml(skill.sourceDisplayName)}
       </span>`;
  } else {
    sourceBadge = `<span class="source-badge local" title="Local skill directory">
         <svg class="source-icon" viewBox="0 0 16 16" width="12" height="12" aria-hidden="true">
           <path fill="currentColor" d="M1.75 1A1.75 1.75 0 000 2.75v10.5C0 14.216.784 15 1.75 15h12.5A1.75 1.75 0 0016 13.25v-8.5A1.75 1.75 0 0014.25 3H7.5a.25.25 0 01-.2-.1l-.9-1.2C6.07 1.26 5.55 1 5 1H1.75z"/>
         </svg>
         Local
       </span>`;
  }

  const lockIcon = isTeam
    ? `<span class="lock-icon" title="Configurado pelo administrador do time">
         <svg width="12" height="12" viewBox="0 0 16 16" fill="currentColor">
           <path d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2zm3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/>
         </svg>
         Bloqueado
       </span>`
    : "";

  return `
    <div class="skill-card ${isTeam ? "locked" : ""}" data-skill="${escapeHtml(skill.name)}">
      <div class="skill-header">
        <span class="skill-name">${escapeHtml(skill.name)}</span>
        <div class="skill-badges">
          ${sourceBadge}
          ${isCustomized && !isTeam ? '<span class="customized-badge">Customized</span>' : ""}
        </div>
      </div>
      <p class="skill-description">${escapeHtml(skill.description)}</p>
      ${!isTeam && skill.sourceType !== "bundled" ? `<div class="skill-path">${escapeHtml(skill.path)}</div>` : ""}
      <div class="skill-controls">
        <div class="toggle-group">
          <span class="toggle-label ${skill.isAssistantOverridden ? "overridden" : ""}">Assistant</span>
          <div
            class="toggle-switch ${skill.assistantInvocable ? "active" : ""} ${isTeam ? "locked" : ""}"
            ${!isTeam ? `data-skill="${escapeHtml(skill.name)}" data-setting="assistant" data-value="${skill.assistantInvocable}"` : ""}
            title="${isTeam ? "Configurado pelo time" : skill.assistantInvocable ? "Model can auto-invoke this skill" : "Model cannot auto-invoke this skill"}"
          ></div>
        </div>
        <div class="toggle-group">
          <span class="toggle-label ${skill.isUserOverridden ? "overridden" : ""}">User</span>
          <div
            class="toggle-switch ${skill.userInvocable ? "active" : ""} ${isTeam ? "locked" : ""}"
            ${!isTeam ? `data-skill="${escapeHtml(skill.name)}" data-setting="user" data-value="${skill.userInvocable}"` : ""}
            title="${isTeam ? "Configurado pelo time" : skill.userInvocable ? "Appears in prompts menu" : "Hidden from prompts menu"}"
          ></div>
        </div>
        ${isTeam ? lockIcon : `
          <button
            class="reset-btn"
            data-skill="${escapeHtml(skill.name)}"
            ${!isCustomized ? "disabled" : ""}
            title="Reset to frontmatter defaults"
          >Reset</button>
        `}
      </div>
    </div>
  `;
}

function attachSkillHandlers(container: HTMLElement) {
  // Add click handlers for toggle switches (not locked)
  container.querySelectorAll(".toggle-switch:not(.locked)").forEach((toggle) => {
    toggle.addEventListener("click", () => {
      const skillName = (toggle as HTMLElement).dataset.skill;
      const setting = (toggle as HTMLElement).dataset.setting as "assistant" | "user";
      const currentValue = (toggle as HTMLElement).dataset.value === "true";
      if (skillName && setting) {
        updateInvocation(skillName, setting, !currentValue);
      }
    });
  });

  // Add click handlers for reset buttons
  container.querySelectorAll(".reset-btn").forEach((btn) => {
    btn.addEventListener("click", () => {
      const skillName = (btn as HTMLButtonElement).dataset.skill;
      if (skillName) {
        resetOverride(skillName);
      }
    });
  });
}

// Tab switching
function switchTab(tab: ActiveTab) {
  activeTab = tab;

  // Update tab button styles
  tabsContainer.querySelectorAll(".tab").forEach((btn) => {
    btn.classList.toggle("active", (btn as HTMLElement).dataset.tab === tab);
  });

  // Update tab content visibility
  document.querySelectorAll(".tab-content").forEach((content) => {
    content.classList.remove("active");
  });
  const activeContent = document.getElementById(`tab-${tab}`);
  if (activeContent) {
    activeContent.classList.add("active");
  }

  render();
}

// Update invocation setting
async function updateInvocation(
  skillName: string,
  setting: "assistant" | "user" | "all",
  value: boolean
) {
  try {
    const result = await app!.callServerTool({
      name: "skill-display-update-invocation",
      arguments: { skillName, setting, value },
    });

    console.log("Update result:", result);

    const structured = result.structuredContent as unknown as SkillDisplayState;
    if (structured?.success) {
      updateState(structured);
      showToast(`${skillName}: ${setting} = ${value ? "on" : "off"}`, "success");
    } else {
      showToast(structured?.error || "Failed to update", "error");
    }
  } catch (error) {
    console.error("Update invocation error:", error);
    showToast((error as Error).message || "Failed to update", "error");
  }
}

// Reset override
async function resetOverride(skillName: string) {
  try {
    const result = await app!.callServerTool({
      name: "skill-display-reset-override",
      arguments: { skillName },
    });

    console.log("Reset result:", result);

    const structured = result.structuredContent as unknown as SkillDisplayState;
    if (structured?.success) {
      updateState(structured);
      showToast(`${skillName} reset to defaults`, "success");
    } else {
      showToast(structured?.error || "Failed to reset", "error");
    }
  } catch (error) {
    console.error("Reset override error:", error);
    showToast((error as Error).message || "Failed to reset", "error");
  }
}

// Toast
function showToast(message: string, type: "success" | "error" = "success") {
  toast.textContent = message;
  toast.className = `toast ${type} visible`;
  setTimeout(() => {
    toast.classList.remove("visible");
  }, 3000);
}

// Utilities
function escapeHtml(str: string): string {
  if (!str) return "";
  return String(str).replace(/[&<>"']/g, (c) =>
    ({ "&": "&amp;", "<": "&lt;", ">": "&gt;", '"': "&quot;", "'": "&#39;" })[c] || c
  );
}

// Set up event listeners
searchInput.addEventListener("input", () => {
  searchQuery = searchInput.value.trim();
  render();
});

// Tab click handlers
tabsContainer.querySelectorAll(".tab").forEach((tab) => {
  tab.addEventListener("click", () => {
    const tabName = (tab as HTMLElement).dataset.tab as ActiveTab;
    if (tabName) {
      switchTab(tabName);
    }
  });
});

hitlApprove.addEventListener("click", () => {
  if (pendingHitlRequest) {
    updateInvocation(pendingHitlRequest.skillName, "all", pendingHitlRequest.enabled);
    pendingHitlRequest = null;
  }
  hitlModal.style.display = "none";
});

hitlDeny.addEventListener("click", () => {
  pendingHitlRequest = null;
  hitlModal.style.display = "none";
  showToast("Agent request denied", "error");
});

function showHitlModal(skillName: string, enabled: boolean) {
  pendingHitlRequest = { skillName, enabled };
  const actionText = enabled ? "enable" : "disable";
  hitlMessage.innerHTML = `The agent has requested to <strong>${actionText}</strong> the skill <strong>${escapeHtml(skillName)}</strong>. Do you allow this?`;
  hitlModal.style.display = "flex";
}

// 1. Create app instance
app = new App({ name: "Skill Display", version: "1.0.0" });

// 2. Register handlers BEFORE connecting
app.onteardown = async () => {
  console.info("App is being torn down");
  return {};
};

app.ontoolinput = (params) => {
  console.info("Received tool input:", params);
};

app.ontoolresult = (result) => {
  console.info("Received tool result:", result);
  if (result.structuredContent) {
    const data = result.structuredContent as any;
    if (data.type === "toggle_request") {
      showHitlModal(data.skillName, data.enabled);
    } else {
      updateState(data as SkillDisplayState);
    }
  }
};

app.ontoolcancelled = (params) => {
  console.info("Tool call cancelled:", params.reason);
};

// ext-apps 1.7.4's App .d.ts no longer surfaces the inherited Protocol.onerror
// (a declaration regression — the runtime and the package's own useApp docs
// still support it). Assign through the base Protocol shape to keep error
// logging without any behavior change.
(app as { onerror?: (error: Error) => void }).onerror = console.error;

app.onhostcontextchanged = handleHostContextChanged;

// 3. Connect to host
app.connect().then(() => {
  console.info("Connected to host");

  // Apply initial host context
  const ctx = app!.getHostContext();
  if (ctx) {
    handleHostContextChanged(ctx);
  }
});
